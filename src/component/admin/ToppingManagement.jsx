...
const ToppingManagement = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      message.error(`Không thể ${editingTopping ? "cập nhật" : "thêm"} topping`)
  ...
  const handleDelete = async (toppingId) => {
    Modal.confirm({
      ...
      content: "Bạn có chắc chắn muốn xóa topping này? Hành động này không thể hoàn tác.",
      okText: "Xóa",
      cancelText: "Hủy",
      okType: "danger",
      onOk: async () => {
    ...
    <div style={{ padding: "24px" }}>
      ...
      >
        <Table
          ...
          locale={{ emptyText: "Chưa có topping nào. Thêm topping để bắt đầu!" }}
      ...
      >
        <Form form={form} onFinish={handleSubmit} layout="vertical" style={{ marginTop: "24px" }}>
          <Form.Item
            name="name"
            label={<span style={{ fontWeight: "600", color: "#1e293b" }}>Tên topping</span>}
            rules={[
              { required: true, message: "Vui lòng nhập tên topping" },
              { min: 2, message: "Tên topping phải có ít nhất 2 ký tự" },
            ]}
          >
            <Input placeholder="Nhập tên topping..." style={{ height: "48px", borderRadius: "8px" }} />
          </Form.Item>
          <Form.Item
            name="price"
            label={<span style={{ fontWeight: "600", color: "#1e293b" }}>Giá (VNĐ)</span>}
            rules={[
