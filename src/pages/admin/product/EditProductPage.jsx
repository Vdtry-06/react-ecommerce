...
const EditProduct = () => {
  ...
  const fetchProduct = async (id) => {
    ...
    try {
      ...
      messageApi.error("Không thể tải dữ liệu sản phẩm");
    } finally {
  ...
  const handleUpdateProductSubmit = async (values) => {
    setLoading(true);
    try {
      ...
      const response = await ApiService.Product.updateProduct(
        productId,
        formData
      );
      if (response.status === 200 || response.status === 201) {
        ...
        messageApi.error("Cập nhật sản phẩm thất bại: Status không hợp lệ");
      }
    } catch (error) {
